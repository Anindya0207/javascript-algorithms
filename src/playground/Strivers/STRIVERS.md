...
### 96. Trajan's Algo: Bridges in Graph
...
### 110. Buy And Sell Stock II 

We need to maximise profit, unlimited transactions but sell has to be done before next buy.
...
Here we have a limit of 2 transactions. Hence, we will also carry forward the transaction Count. Once it reaches 2 we return

```javascript
var maxProfit = function (prices) {
    let dp = Array.from({length: prices.length}, () => Array.from({length: 2}, () => undefined))
    const _calc = (index, buy, transactionCount) => {
        if(index == prices.length || transactionCount == 2) return 0;
        if(dp[index][buy] != undefined) return dp[index][buy];
        if(buy) {
            dp[index][buy]= Math.max(
                -prices[index] + _calc(index + 1, 0, transactionCount ),
                0 + _calc(index +1, 1,  transactionCount )
            )
        } else {
            dp[index][buy]= Math.max(
                prices[index] + _calc(index + 1, 1,  transactionCount + 1), // sell kar rhe, transaction khatam hua ek
                0 + _calc(index +1, 0,  transactionCount)
            )
        }
        return dp[index][buy]
    }
